...
func New(settings ...*Settings) Burn {
	gz := new(burn)
...
func (gz *burn) Start(address string) error {
	// Setup router
	gz.setupRouter()
...
func (gz *burn) newHTTPServer() *fasthttp.Server {
...
func (gz *burn) registerRoute(method, path string, handlers handlersChain) *Route {
...
func (gz *burn) Static(prefix, root string) {
	if gz.settings.CaseInSensitive {
...
// match with request
func (gz *burn) NotFound(handlers ...handlerFunc) {
	gz.router.SetNotFound(handlers)
}

func (gz *burn) Use(middlewares ...handlerFunc) {
